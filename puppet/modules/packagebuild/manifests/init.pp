...
class packagebuild {
  ...
  $ruby_pkgs = [ 'ruby1.9.1-dev', 'ruby1.9.1' ]
  ...
  exec { 'install_gedoce':
    command  => "/usr/bin/sudo USE_SYSTEM_GECODE=1 /usr/bin/gem install dep-selector-libgecode",
    user  => "root",
  }
  ...
  $build_utils = [ 'build-essential', 'libpcre3-dev', 'unzip', 'libxml2-dev', 'libxslt1-dev', 'libgd2-xpm-dev', 'libgeoip-dev', 'python-setuptools', 'libgecode-dev', 'autoconf', 'libtool' ]
